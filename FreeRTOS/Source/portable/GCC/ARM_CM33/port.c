...
/*-----------------------------------------------------------
 * Implementation of functions defined in portable.h for the ARM CM33F port.
 * TODO: some day it might be nice to use PSPLIM/MSPLIM
 *----------------------------------------------------------*/
...
// Using real FPU
		...
		"       ldr r14, [r2, #0x4]\n" /* save MPU_CTRL */
		"       mov r4, #0\n"
		"       str r4, [r2, #0x4]\n" /* turn off the MPU while loading new registers, since this is necessarily non-atomic */
		"       mov r4, #4\n"
		"       str r4, [r2, #0x8]\n" /* MPU_RNR = 4 */
		"	ldr r2, =0xe000ed9c					\n" /* Region Base Address register. */
		"	ldmia r1!, {r4-r11}					\n" /* Read 4 sets of MPU registers. */
...
uint32_t ulParameters;
	...
	for( ul = 0; ul < portNUM_CONFIGURABLE_REGIONS; ul++ )
	{
		 ...
		 * written into the MPU_RLAR register, and the base address is the RBAR for the region */
